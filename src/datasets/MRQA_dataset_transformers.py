...
from src.datasets.utility import TRAIN_F_MRQA_SQUAD, VALIDATION_F_MRQA_SQUAD, TRAIN_F_MRQA_NQ, VALIDATION_F_MRQA_NQ,TRAIN_F_MRQA_NEWSQA, VALIDATION_F_MRQA_NEWSQA,TRAIN_F_MRQA_TRIVIAQA, VALIDATION_F_MRQA_TRIVIAQA,download_url, TRAIN_V1_URL_SQUAD, \
    DEV_V1_URL_SQUAD, \
    JOIN_TOKEN, find_sub_list
...
def squad_convert_example_to_features(example, max_seq_length, doc_stride, max_query_length, is_training):
    ...
    if is_training and not example.is_impossible:
        ...
        if actual_text.lower().find(cleaned_answer_text.lower()) != -1 or actual_text.find(cleaned_answer_text) != -1:
            pass
        else:
            #TODO: ?????
            start_position = example.start_position-1
            end_position = example.end_position-1
            actual_text = " ".join(example.doc_tokens[start_position: (end_position + 1)])
            if actual_text.lower().find(cleaned_answer_text.lower()) != -1 or actual_text.find(cleaned_answer_text)  != -1:
                logger.debug("Had to shift start position, but why?: '%s' vs. '%s'", actual_text, cleaned_answer_text)
                #return []
            else:
...
class SquadProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, filename=None):
        ...
        input_data=[]
        with jsonlines.open(filename, "r") as fd:
            for paragraph in fd:
    ...
    def get_dev_examples(self, data_dir, filename=None):
        ...
        input_data=[]
        with jsonlines.open(os.path.join(data_dir, self.dev_file if filename is None else filename), "r") as fd:
            for paragraph in fd:
                if "header" in paragraph:
                    continue
                input_data.append(paragraph)
                 ...
                 #   if is_training:
                  #      answer = qa['detected_answers'][0]
                   #     answer_text = answer["text"]
                    #    start_position_character = answer["char_spans"][0][0]#["answer_start"]
                    #else:
                     #   answers = qa["detected_answers"] [0]
...
class MRQADataset_transformers(data.Dataset):
    def __init__(self, data_f: string, tokenizer: PreTrainedTokenizer, fields: List[Tuple[str, data.Field]],
                 ...
                 cachedir='.data/mrqa', max_len=512 - 3, **kwargs):
        ...
        preprocessed_f_noext = os.path.join(cachedir, os.path.basename(data_f)) + f"_preprocessed_for_{transformer}_transformers"
        ...
        super(MRQADataset_transformers, self).__init__(examples, fields, **kwargs)
    ...
    @classmethod
    def splits(cls, tokenizer, fields, subdataset, cachedir='.data/mrqa'):
        cls.check_for_download(cachedir)
        if subdataset=="squad":
            train_data = cls(TRAIN_F_MRQA_SQUAD, tokenizer, fields, cachedir=cachedir)
            val_data = cls(VALIDATION_F_MRQA_SQUAD, tokenizer, fields, cachedir=cachedir, offset=val_offset)
        elif subdataset=="nq":
            train_data = cls(TRAIN_F_MRQA_NQ, tokenizer, fields, cachedir=cachedir)
            val_data = cls(VALIDATION_F_MRQA_NQ, tokenizer, fields, cachedir=cachedir, offset=val_offset)
        elif subdataset=="nq":
            train_data = cls(TRAIN_F_MRQA_NEWSQA, tokenizer, fields, cachedir=cachedir)
            val_data = cls(VALIDATION_F_MRQA_NEWSQA, tokenizer, fields, cachedir=cachedir, offset=val_offset)
        elif subdataset=="triviaqa":
            train_data = cls(TRAIN_F_MRQA_TRIVIAQA, tokenizer, fields, cachedir=cachedir)
            val_data = cls(VALIDATION_F_MRQA_TRIVIAQA, tokenizer, fields, cachedir=cachedir, offset=val_offset)
    ...
    def check_for_download(cachedir):
        if not os.path.exists(cachedir):
            ...
            sys.stderr.write(f'Dataset not found')
            sys.stderr.flush()
            raise BaseException
