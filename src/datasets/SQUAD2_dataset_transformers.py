...
from transformers import squad_convert_examples_to_features, PreTrainedTokenizer, SquadFeatures, SquadV2Processor

from src.datasets.utility import download_url, VALIDATION_F_SQUAD2, TRAIN_F_SQUAD2, TRAIN_V2_URL_SQUAD, \
    DEV_V2_URL_SQUAD


class Squad2Dataset_transformers(data.Dataset):
    def __init__(self, data_f: string, tokenizer: PreTrainedTokenizer, fields: List[Tuple[str, data.Field]],
                 transformer,
                 is_training,
                 cachedir='.data/squad2', max_len=512 - 3, **kwargs):
        ...
        super(Squad2Dataset_transformers, self).__init__(examples, fields, **kwargs)
    ...
    @classmethod
    def splits(cls, tokenizer, fields, cachedir='.data/squad2'):
        cls.check_for_download(cachedir)
        train_data = cls(TRAIN_F_SQUAD2, tokenizer, fields, cachedir=cachedir)
        val_data = cls(VALIDATION_F_SQUAD2, tokenizer, fields, cachedir=cachedir)
    ...
    def check_for_download(cachedir):
        if not os.path.exists(cachedir):
            ...
            try:
                download_url(os.path.join(cachedir, TRAIN_F_SQUAD2), TRAIN_V2_URL_SQUAD)
                download_url(os.path.join(cachedir, VALIDATION_F_SQUAD2), DEV_V2_URL_SQUAD)
