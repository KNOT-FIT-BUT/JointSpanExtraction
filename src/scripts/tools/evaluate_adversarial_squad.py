...
def evaluate_advsquad(df, pf, verbose=False):
    with open(df) as dataset_file:
        dataset_json = json.load(dataset_file)
        dataset = dataset_json['data']
    with open(pf) as prediction_file:
        predictions = json.load(prediction_file)
    if verbose:
        from termcolor import colored
    return evaluate_adversarial(dataset, predictions, verbose=verbose)
